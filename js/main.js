...
function drawPlanet(planet, date) {
  ...
  if (planet.name === "Sun") {
    drawSun(drawX, drawY, drawRadius, planet.color);
  }
  // Special case for Saturn - add rings
  else if (planet.name === "Saturn") {
    drawSaturn(drawX, drawY, drawRadius, planet.color);
  }
  // Earth - add atmosphere effect and moon
  else if (planet.name === "Earth") {
    drawEarth(drawX, drawY, drawRadius, planet.color, date);
  }
  // All other planets
  else {
  ...
  if (
    ...
    drawDistanceLine(planet, position, drawX, drawY, drawRadius, date);
  }
...
function drawSaturn(x, y, radius, color) {
  ctx.fillStyle = color;
  ctx.fill();
  ...
  ctx.ellipse(
    x,
    y,
    radius + SATURN_RING_SIZE * zoom,
    (radius + SATURN_RING_SIZE * zoom) * 0.3,
    Math.PI / 6,
    0,
    2 * Math.PI
  );
...
// Draw Earth with atmosphere and moon
function drawEarth(x, y, radius, color, date) {
  ...
  atmosphere.addColorStop(0, color);
  ...
  const moonPos = {
    x: x + MOON_ORBIT_RADIUS * zoom * Math.cos(moonAngle),
    y: y + MOON_ORBIT_RADIUS * zoom * Math.sin(moonAngle),
  };
...
// Draw distance line between planet and sun
function drawDistanceLine(planet, position, drawX, drawY, drawRadius, date) {
...
function handlePlanetSelection(event) {
  ...
  for (let i = planets.length - 1; i >= 0; i--) {
    ...
    const distance = Math.sqrt(
      Math.pow(clickX - planet.drawnX, 2) + Math.pow(clickY - planet.drawnY, 2)
    );
...
function handleSpeedChange() {
  ...
  if (daysPerSecond >= 365) {
    speedValue.textContent = `${(daysPerSecond / 365.25).toFixed(1)} years/sec`;
  } else {
...
// Handle current position button
...
function init() {
  ...
  currentPositionButton.addEventListener("click", handleCurrentPosition);
  speedSlider.addEventListener("input", handleSpeedChange);
  playPauseButton.addEventListener("click", handlePlayPause);
  resetButton.addEventListener("click", handleReset);

  // Canvas event listeners
  canvas.addEventListener("click", handlePlanetSelection);
